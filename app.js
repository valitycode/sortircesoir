...
function renderAdminMap(locations) {
  ...
  if (!validLocations.length) {
    if (adminLeafletMap) {
      ...
      adminLeafletMarkers = null;
    }
    mapEl.classList.remove('leaflet-container');
    mapEl.innerHTML = '<div class="admin-map-fallback">Aucune position partagée pour le moment.</div>';
    return;
  }

  if (!window.L) {
    mapEl.innerHTML = '<div class="admin-map-fallback">Carte indisponible : Leaflet ne s’est pas chargé.</div>';
    return;
  }

  if (!adminLeafletMap) {
    mapEl.innerHTML = '';
    adminLeafletMap = L.map(mapEl, {
      zoomControl: true,
    ...
    adminLeafletMarkers = L.layerGroup().addTo(adminLeafletMap);
  }

  adminLeafletMarkers.clearLayers();
  ...
  validLocations.forEach(l => {
    ...
    const userLabel = escapeHTML(profileName(l.profiles) || l.profiles?.username || 'Utilisateur');
    const username = escapeHTML(l.profiles?.username || '');
    const updated = l.updated_at ? new Date(l.updated_at).toLocaleString('fr-FR') : '';

    const marker = L.marker([lat, lng]).addTo(adminLeafletMarkers);
    marker.bindPopup(`
      <div class="map-popup">
        <b>${userLabel}</b><br>
        ${username ? `<span>@${username}</span><br>` : ''}
        <small>${lat.toFixed(5)}, ${lng.toFixed(5)}</small><br>
        <small>${escapeHTML(updated)}</small><br><br>
        <button class="btn dark" onclick="adminOpenUser('${l.user_id}')">Voir le compte</button>
      </div>
    `);
  });

  if (bounds.length === 1) {
    adminLeafletMap.setView(bounds[0], 13);
  } else {
    adminLeafletMap.fitBounds(bounds, { padding: [34, 34], maxZoom: 14 });
  }
